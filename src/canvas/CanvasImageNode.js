...
export function CanvasImageNode({ obj, isSelected, onSelect, onGuidesChange }) {
    ...
    const stableImageRef = useRef(undefined);
    ...
    const groupClip = useMemo(() => ({ x: 0, y: 0, width: obj.frameWidth, height: obj.frameHeight }), [obj.frameWidth, obj.frameHeight]);
    ...
    function handleDblClick() {
        if (!obj.locked)
            commitUpdate(obj.id, { contentEditMode: true });
    }
    function handleFrameDragMove(e) {
        const rect = e.target;
        const rawX = rect.x();
        const rawY = rect.y();
        const { x: snappedX, y: snappedY, guides } = computeSnap({ x: rawX, y: rawY, width: obj.frameWidth, height: obj.frameHeight }, obj.id);
        rect.x(snappedX);
        rect.y(snappedY);
    ...
    function handleFrameTransformEnd(e) {
        const rect = frameRectRef.current;
        if (!rect)
            return;
        const isPureRotation = Math.abs(rect.scaleX() - 1) < 0.001 && Math.abs(rect.scaleY() - 1) < 0.001;
        ...
        const cmdFromEvent = 'metaKey' in nativeEvent && nativeEvent.metaKey;
        ...
        if (isPropMode && !isPureRotation) {
            ...
            if (img) {
                img.x(newContentOffsetX);
                img.y(newContentOffsetY);
            }
            commitUpdate(obj.id, {
                frameX: newFrameX,
                frameY: newFrameY,
    ...
    function handleContentTransformEnd() {
        const img = imageRef.current;
        if (!img)
            return;
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Group, { ref: groupRef, x: obj.frameX, y: obj.frameY, clip: groupClip, rotation: obj.rotation, opacity: (obj.maskEditMode || isDrawTarget) ? obj.opacity * 0.5 : obj.opacity, listening: obj.contentEditMode && !isDrawTarget, children: _jsxs(Group, { ref: innerGroupRef, children: [_jsx(KonvaImage, { ref: imageRef, image: image, x: obj.contentOffsetX, y: obj.contentOffsetY, width: obj.contentWidth, height: obj.contentHeight, draggable: obj.contentEditMode && !obj.locked, onClick: () => { if (obj.contentEditMode)
                                onSelect(); }, onTap: () => { if (obj.contentEditMode)
                                onSelect(); }, onDragEnd: handleContentDragEnd, onTransformEnd: handleContentTransformEnd }), maskSceneFunc !== undefined && (_jsx(Shape, { sceneFunc: maskSceneFunc, globalCompositeOperation: "destination-in", listening: false }))] }) }), obj.maskEditMode && obj.mask && (_jsx(Group, { x: obj.frameX, y: obj.frameY, rotation: obj.rotation, listening: true, children: obj.mask.anchors.map((anchor, i) => {
                    ...
                    const mask = obj.mask;
                    return (_jsxs(React.Fragment, { children: [hasHandleIn && (_jsxs(_Fragment, { children: [_jsx(Line, { points: [ax, ay, hix, hiy], stroke: "#0096ff", strokeWidth: 1, listening: false }), _jsx(Circle, { x: hix, y: hiy, radius: 4, fill: "#fff", stroke: "#0096ff", strokeWidth: 1, draggable: true, onDragMove: (e) => {
                                            const n = e.target;
                                            const newAnchors = mask.anchors.map((a, j) => j === i ? { ...a, handleIn: { dx: n.x() - ax, dy: n.y() - ay } } : a);
                                            updateObject(obj.id, { mask: { ...mask, anchors: newAnchors } });
                                        }, onDragEnd: (e) => {
                                            const n = e.target;
                                            const newAnchors = mask.anchors.map((a, j) => j === i ? { ...a, handleIn: { dx: n.x() - ax, dy: n.y() - ay } } : a);
                                            commitUpdate(obj.id, { mask: { ...mask, anchors: newAnchors } });
                                        } })] })), hasHandleOut && (_jsxs(_Fragment, { children: [_jsx(Line, { points: [ax, ay, hox, hoy], stroke: "#0096ff", strokeWidth: 1, listening: false }), _jsx(Circle, { x: hox, y: hoy, radius: 4, fill: "#fff", stroke: "#0096ff", strokeWidth: 1, draggable: true, onDragMove: (e) => {
                                            const n = e.target;
                                            const newAnchors = mask.anchors.map((a, j) => j === i ? { ...a, handleOut: { dx: n.x() - ax, dy: n.y() - ay } } : a);
                                            updateObject(obj.id, { mask: { ...mask, anchors: newAnchors } });
                                        }, onDragEnd: (e) => {
                                            const n = e.target;
                                            const newAnchors = mask.anchors.map((a, j) => j === i ? { ...a, handleOut: { dx: n.x() - ax, dy: n.y() - ay } } : a);
                                            commitUpdate(obj.id, { mask: { ...mask, anchors: newAnchors } });
                                        } })] })), _jsx(Circle, { x: ax, y: ay, radius: 5, fill: "#0096ff", stroke: "#fff", strokeWidth: 1.5, draggable: true, onDragMove: (e) => {
                                    const n = e.target;
                                    const newAnchors = mask.anchors.map((a, j) => j === i ? { ...a, x: n.x() - obj.contentOffsetX, y: n.y() - obj.contentOffsetY } : a);
                                    updateObject(obj.id, { mask: { ...mask, anchors: newAnchors } });
                                }, onDragEnd: (e) => {
                                    const n = e.target;
                                    const newAnchors = mask.anchors.map((a, j) => j === i ? { ...a, x: n.x() - obj.contentOffsetX, y: n.y() - obj.contentOffsetY } : a);
                ...
                }) })), _jsx(Rect, { ref: frameRectRef, x: obj.frameX, y: obj.frameY, width: obj.frameWidth, height: obj.frameHeight, rotation: obj.rotation, fill: "transparent", stroke: "#0096ff", strokeWidth: 1, strokeEnabled: obj.contentEditMode || isInMultiSelect, strokeScaleEnabled: false, perfectDrawEnabled: false, draggable: !obj.locked && !obj.contentEditMode && !obj.maskEditMode && !isDrawTarget, listening: !obj.contentEditMode && !obj.maskEditMode && !isDrawTarget, onClick: (e) => {
                ...
                }, onTap: () => { if (!obj.contentEditMode && !obj.maskEditMode)
                    onSelect(); }, onDblClick: handleDblClick, onDblTap: handleDblClick, onDragStart: () => {
                    dragStartFrameXRef.current = obj.frameX;
                    dragStartFrameYRef.current = obj.frameY;
                    pendingDuplicateRef.current = false;
                }, onDragMove: handleFrameDragMove, onDragEnd: handleFrameDragEnd, onTransform: () => {
                    syncGroupOnTransform();
                    onGuidesChange(pendingGuidesRef.current);
                }, onTransformEnd: handleFrameTransformEnd, onContextMenu: (e) => {
