 ...
 */
export async function exportFrames(stage, frameCount, frameWidth, frameHeight, startFrame = 0, endFrame = frameCount - 1) {
    ...
    textNodes.forEach((n) => {
        const userOpacity = n.getAttr('userOpacity');
    ...
    try {
        ...
        for (let i = startFrame; i <= endFrame; i++) {
            ...
            if (!ctx)
                throw new Error(`Failed to get 2d context for frame ${i}`);
            ctx.drawImage(fullCanvas, i * frameWidth * PIXEL_RATIO, 0, // source origin
 ...
 */
export async function downloadFrames(blobs) {
