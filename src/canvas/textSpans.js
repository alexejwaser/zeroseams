...
export function fontStyleToCSS(fontStyle) {
    switch (fontStyle) {
...
export function spanText(obj) {
    return obj.spans.map((s) => s.text).join('');
}
export function resolveSpanStyle(obj, spanIdx) {
...
export function getSelectionStyle(obj, start, end) {
    if (start >= end) {
 ...
 */
function splitSpansAt(spans, positions) {
    ...
    for (const pos of sortedPositions) {
        ...
        for (let i = 0; i < result.length; i++) {
            ...
            if (pos > spanStart && pos < spanEnd) {
                ...
                const left = { text: span.text.slice(0, splitAt), style: span.style };
                const right = { text: span.text.slice(splitAt), style: span.style };
 ...
 */
export function applyStyleToRange(obj, start, end, style) {
    if (start >= end)
        return obj.spans;
 ...
 */
export function applyStyleToAll(obj, style) {
    const keysToReset = Object.keys(style);
